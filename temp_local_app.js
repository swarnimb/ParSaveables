...
async function renderPodcastPage(container) {
    ...
    episodesContainer.className = 'podcast-episodes';

    try {
        // Fetch episodes from GitHub Releases
        const response = await fetch('https://api.github.com/repos/swarnimb/ParSaveables/releases');
        const releases = await response.json();

        // Filter for podcast episodes (check for .mp3 assets)
        ...
        if (podcastReleases.length === 0) {
            ...
            emptyState.innerHTML = `
                <p>No episodes yet. The first episode will be generated automatically!</p>
                <p class="podcast-empty-sub">Check back soon for disc golf chaos.</p>
            `;
            episodesContainer.appendChild(emptyState);
        } else {
            // Render episodes
            podcastReleases.forEach(release => {
                const mp3Asset = release.assets.find(asset => asset.name.endsWith('.mp3'));
                if (!mp3Asset) return;

                const episodeCard = document.createElement('div');
                episodeCard.className = 'podcast-episode-card';

                // Extract episode number from tag (e.g., "podcast-ep1")
                const episodeMatch = release.tag_name.match(/ep(\d+)/i);
                ...
                episodeCard.innerHTML = `
                    <div class="podcast-episode-header">
                        <div class="podcast-episode-number">EP ${episodeNumber}</div>
                        <div class="podcast-episode-date">${publishedDate}</div>
                    </div>
                    <div class="podcast-episode-title">${release.name.replace(/🎙️|Episode \d+:|Podcast Episode \d+:/gi, '').trim()}</div>
                    <audio controls class="podcast-player">
                        <source src="${mp3Asset.browser_download_url}" type="audio/mpeg">
        ...
        errorState.className = 'podcast-empty';
        errorState.innerHTML = `
            <p>Unable to load episodes. Please try again later.</p>
        `;
        episodesContainer.appendChild(errorState);
    }
