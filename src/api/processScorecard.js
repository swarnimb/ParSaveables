 ...
 * Ties together all 8 services in a 12-step workflow
 *
...
async function processSingleEmail(email, options = {}) {
  ...
  // Step 11: Mark email as processed
  logger.info('Step 11: Marking email as processed');
  await emailService.markAsProcessed(email.id);
  await emailService.addLabel(email.id, 'ParSaveables/Processed');

  // Step 12: Send success notification
  if (!skipNotifications) {
    logger.info('Step 12: Sending success notification');
    try {
      const senderEmail = extractEmailAddress(email.from);
